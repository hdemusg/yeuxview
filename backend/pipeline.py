...
def classify(image_l, image_r):
    ...
    '''
    '''
    mydict = pickle.load(open('export.pkl', 'r'))
    f.close()
    im_l = mydict.predict(tensor(image_l))
    im_r = mydict.predict(tensor(image_r))
    '''  
    learn_test = load_learner('export.pkl')
    im_l = learn_test.predict(tensor(image_l))
    im_r = learn_test.predict(tensor(image_r))
    ...
    print(type(data))
    print(data)
    res = jsonify({'image1': {'preds': data[0][1].tolist(), 'probs': data[0][2].tolist()}, 'image2': {'preds': data[1][1].tolist(), 'probs': data[1][2].tolist()}})
    return res
